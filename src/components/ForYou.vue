    ...
    <vue-horizontal ref="horizontal" class="horizontal" responsive v-if="products">
      ...
      <div v-for="product in products" :key="product.id" class="flex flex-row px-5">
        ...
        <a :href="$router.resolve({name: 'Product', params: { id: product.id, slug: sanitizeTitle(`${product.name}`) }}).href"
          class="w-60 h-72">
          <img v-if="product" v-bind:src="product.photo"
            class="w-56 mx-auto transition duration-500 ease-in-out transform h-52 hover:-translate-y-1 hover:scale-100 rounded-t-xl">
          <div class="p-3">
            ...
            <div class="flex justify-between">
              <div>
                <span v-if="product.discount_price > 0">
                  <p class="text-xs text-gray-700 ">{{ currency }}<span class="mr-1 text-base">{{
                  product.discount_price.toFixed(1) }}</span> <span class="line-through">{{ product.price.toFixed(1)
                      }}</span></p>
                </span>
                <span v-else>
