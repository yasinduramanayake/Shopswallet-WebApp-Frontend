<template>
  <div class="px-2">
    <div class="flex justify-between my-10">
      <p class="text-2xl font-semibold text-gray-700">{{ $t('pages.todaysPicks') }}</p>
      <!-- <div>
        <nav>
    ...
    <vue-horizontal ref="horizontal" class="horizontal" responsive v-if="products">
        ...
        <div v-for="product in products" :key="product.id" class="flex flex-row px-5 mb-10">
          ...
          <a :href="$router.resolve({name: 'Product', params: { id: product.id, slug: sanitizeTitle(`${product.name}`) }}).href" class="w-60 h-88">
            <img  v-if="product" v-bind:src="product.photo" class="mx-auto transition duration-500 ease-in-out transform w-60 h-60 hover:-translate-y-1 hover:scale-100 rounded-t-xl">
            <div class="p-3 ">
              <div class="flex justify-between">
                ...
                <div>
                  <div class=""  v-if="product.available_qty > 0">
                    <p class="text-xs ">
                        Instock: <span class="truncate">{{ product.available_qty }}</span>
                    </p>
                  </div>
                  <div v-else>
                    <p class="text-xs ">
              ...
              <div class="flex justify-between">
                ...
                <div>
                  <div class="px-2 py-1 text-xs text-white bg-yellow-600 rounded-full">Best Seller</div>
                </div>
