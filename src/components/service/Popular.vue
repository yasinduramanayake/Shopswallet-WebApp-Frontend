<template>
  <div class="px-2">
    <div class="flex justify-between my-10">
      <p class="text-2xl font-semibold text-gray-700 md:text-4xl">{{ $t('pages.popularServices') }}</p>
      <!-- <div>
        <nav>
    ...
    <vue-horizontal ref="horizontal" class="horizontal" responsive v-if="products">
        <div v-for="product in products" :key="product.id" class="flex flex-row px-5 mb-10">
          ...
          <a :href="$router.resolve({name: 'ServiceProduct', params: { id: product.id, slug: sanitizeTitle(`${product.name}`) }}).href" class="w-60 h-60">
...
export default {
  ...
  mounted () {
    ...
    axios.get(this.base_url+'api/services?type=popular&vendor_type_id='+this.$route.params.id)
    .then((response) => 
      {
