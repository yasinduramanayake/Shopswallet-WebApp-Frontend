<template>
  <div class="px-2">
    <div class="flex justify-between mb-10">
        <p class="text-2xl font-semibold text-gray-700">{{ $t('pages.books') }}</p>
          ...
          <a :href="$router.resolve({name: 'Product', params: { id: product.id, slug: sanitizeTitle(`${product.name}`) }}).href" class="w-60 h-68">
            ...
            <div class="p-3 ">
              ...
              <div class="flex justify-between">
                ...
                <div>
                  <div class="px-2 py-1 text-xs text-white bg-red-300 rounded-full">{{ $t('pages.book') }}</div>
                </div>
...
export default {
  ...
  mounted () {
    ...
    axios.get(this.base_url+'api/products?type=book&vendor_type_id='+this.$route.params.id)
    .then((response) => {
